...
import DATA_MODEL from "../../models/restaurant/promotionModel.ts";
...
const services = {
  getData: async function (filter: any, select?: any, sort: any = {}, skip: any = 0, limit: any = 10) {
    ...
    try {
      data = await DATA_MODEL.find(filter)
  ...
  getSingleDocument: async function (filter: any, select?: any) {
    ...
    try {
      data = await DATA_MODEL.findOne(filter)
        .read("secondaryPreferred")
        .select(select)
        .lean();
    } catch (e: any) {
  ...
  getCountDocument: async (filter: any) => {
    ...
    try {
      count =
        await DATA_MODEL.countDocuments(filter).read("secondaryPreferred");
    } catch (e: any) {
      console.error(e.message);
    }

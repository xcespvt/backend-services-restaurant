...
import MenuService from "../../services/restaurant/menuServices.ts";
...
const menuController = {
  ...
  deleteCloudflareImage: async (request: FastifyRequest | any, reply: FastifyReply) => {
    try {
      ...
      if (!(data as any).success) {
        throw new Error((data as any).errors?.[0]?.message || 'Failed to delete image from Cloudflare');
      }
  ...
  uploadImageToCloudflare: async (request: FastifyRequest | any, reply: FastifyReply) => {
    try {
      ...
      if (!(data as any).success) {
        throw new Error((data as any).errors?.[0]?.message || "Cloudflare upload failed");
      }

      return reply.code(200).send({
        success: 1,
        message: "Image uploaded successfully",
        imageId: (data as any).result.id,
        variants: (data as any).result.variants,
      });
    } catch (error: any) {
  ...
  getMenuItems: async (request: FastifyRequest | any, reply: FastifyReply) => {
    try {
  ...
  addMenuItem: async (request: FastifyRequest | any, reply: FastifyReply) => {
    try {
  ...
  updateMenuItem: async (request: FastifyRequest | any, reply: FastifyReply) => {
    try {
  ...
  deleteMenuItem: async (request: FastifyRequest | any, reply: FastifyReply) => {
    try {
      const { restaurantId, itemId } = request.params;
  ...
  toggleMenuItemAvailability: async (request: FastifyRequest | any, reply: FastifyReply) => {
    try {
  ...
  addCategory: async (request: FastifyRequest | any, reply: FastifyReply) => {
    try {
  ...
  searchMenuItems: async (request: FastifyRequest | any, reply: FastifyReply) => {
    try {
      const { restaurantId } = request.params;
      if (!restaurantId) {
        return reply.code(400).send({
      ...
      let filter: any = { restaurantId: { $eq: restaurantId } };

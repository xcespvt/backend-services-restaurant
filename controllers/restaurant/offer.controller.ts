...
import OfferService from "../../services/restaurant/offerService.ts";
import { v7 as uuidv7 } from "uuid";

const offerController = {
  getOffers: async (request: FastifyRequest | any, reply: FastifyReply) => {
    try {
  ...
  addOffers: async (request: FastifyRequest | any, reply: FastifyReply) => {
        try {
            const {
                restaurantId,
                couponCode,
  ...
  updateOffers: async (request: FastifyRequest | any, reply: FastifyReply) => {
    try {
      ...
      const offers = await OfferService.getData({ restaurantId, offerId });
      const existingOffer = offers && offers.length > 0 ? (offers[0] as any) : null;
      ...
      const validFieldsByType: Record<string, string[]> = {
  ...
  deleteOffers: async (request: FastifyRequest | any, reply: FastifyReply) => {
        try {
            const { restaurantId, offerId } = request.params;
  ...
  toggleOfferAvailability: async (request: FastifyRequest | any, reply: FastifyReply) => {
  ...
  addOfferCategory: async (request: FastifyRequest | any, reply: FastifyReply) => {
        try {
            const { restaurantId } = request.params;

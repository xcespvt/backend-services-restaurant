...
import tableTypeService from "../../services/restaurant/tabletypeService.ts";
import { v7 as uuidv7 } from 'uuid';

const tableTypeController = {
  addTableType: async (request: any, reply: any) => {
    try {
  ...
  getTableTypes: async (request: any, reply: any) => {
    try {
      const { branchId } = request.params;
      if (!branchId) {
        return reply.code(400).send({
  ...
  updateTableType: async (request: any, reply: any) => {
    try {
  ...
  deleteTableType: async (request: any, reply: any) => {
    try {
      const { branchId, typeId } = request.params;

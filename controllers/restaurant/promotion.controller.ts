...
import PromotionService from "../../services/restaurant/promotionService.ts";
import { v7 as uuidv7 } from "uuid";

const promotionController = {
  getPromotions: async (request: FastifyRequest | any, reply: FastifyReply) => {
    try {
  ...
  addPromotions: async (request: FastifyRequest | any, reply: FastifyReply) => {
    try {
      const {
        restaurantId,
        campaignTitle,
      ...
      if (placementOptions) {
        for (const key of validPlacements) {
          const placement = (placementOptions as any)[key];
  ...
  updatePromotions: async (request: FastifyRequest | any, reply: FastifyReply) => {
    try {
      ...
      if (updateData.placementOptions) {
        ...
        for (const key of validPlacements) {
          const placement = (updateData.placementOptions as any)[key];
          if (placement?.isEnabled && placement?.price) {
            totalCost += placement.price;
          }
        }
  ...
  deletePromotions: async (request: FastifyRequest | any, reply: FastifyReply) => {
    try {
      const { restaurantId, promotionId } = request.params;
  ...
  togglePromotionAvailability: async (request: FastifyRequest | any, reply: FastifyReply) => {
    try {
      const { restaurantId, promotionId } = request.params;
  ...
  addPromotionCategory: async (request: FastifyRequest | any, reply: FastifyReply) => {
    try {
      const { restaurantId } = request.params;

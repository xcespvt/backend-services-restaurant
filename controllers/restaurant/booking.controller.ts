...
import bookingService from "../../services/restaurant/bookingService.ts";
import { v7 as uuidv7 } from 'uuid';

const bookingController = {
  getAllBookings: async (request: FastifyRequest | any, reply: FastifyReply) => {
    try {
      ...
      const formattedBookings: Record<string, any> = {};
  ...
  addTableSeries: async (request: FastifyRequest | any, reply: FastifyReply) => {
    try {
      const { restaurantId } = request.params;
      ...
      return reply.code(201).send({
        success: true,
        total: (created as any).length,
        message: `Created ${(created as any).length} tables successfully`,
        data: created
      });
  ...
  addTable: async (request: FastifyRequest | any, reply: FastifyReply) => {
    try {
      const { restaurantId } = request.params;
  ...
  updateTable: async (request: FastifyRequest | any, reply: FastifyReply) => {
    try {
  ...
  deleteTable: async (request: FastifyRequest | any, reply: FastifyReply) => {
    try {
      const { restaurantId, tableId } = request.params;
  ...
  updateTableStatus: async (request: FastifyRequest | any, reply: FastifyReply) => {
    try {
      const { restaurantId, tableId } = request.params;
